...
const Understand = () => {
  return (
    ...
    {  
      [10,20,30,40].map((item:number,index:number)=>(<img key={index} src={lines} className={`absolute ${`md:translate-x-[${item}%]`}   lg:translate-x-[${item+10}%]  hidden md:block h-full`}alt="lines" />))}
