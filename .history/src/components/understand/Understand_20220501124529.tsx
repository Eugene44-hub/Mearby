...
const Understand = () => {
  return (
    ...
    {  
      [1,3,5,7].map((item:number,index:number)=>(<img key={index} src={lines} className={`absolute ${`md:translate-x-[${item*5}%]`}   lg:translate-x-[${(item*10)+30}%]  hidden md:block h-full`}alt="lines" />))}
