...
const Understand = () => {
  ...
  return (
    ...
    {  
      [20,40,60,80].map((item,index:number)=>(<img key={index} src={lines} className={`absolute ${`md:translate-x-[${item.md}%]`}  
      lg:translate-x-[${item.lg}%] lg:ml-[10rem]  hidden md:block h-full`}alt="lines" />))}
     {/* <img src={lines} className='absolute left-[20%] hidden md:block h-full' alt="lines" />
     <img src={lines} className='absolute left-[40%] hidden md:block   h-full' alt="lines" />
     <img src={lines} className='absolute left-[60%] hidden md:block  h-full' alt="lines" />
     <img src={lines} className='absolute left-[80%] hidden md:block  h-full' alt="lines" /> */}
