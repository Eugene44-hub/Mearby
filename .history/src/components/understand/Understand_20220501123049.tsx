...
const Understand = () => {
  return (
    ...
    {  
      [1,3,5,70].map((item:number,index:number)=>(<img key={index} src={lines} className={`absolute ${`md:translate-x-[${item}%]`}   lg:translate-x-[${item+50}%]  hidden md:block h-full`}alt="lines" />))}
