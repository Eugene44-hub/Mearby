...
const Understand = () => {
  return (
    ...
    {  
      [10,30,50,70].map((item:number,index:number)=>(<img key={index} src={lines} className={`absolute ${`md:translate-x-[{item}]`} lg:translate-x-[40%] ${`md:translate-x-[{item}]`}  hidden md:block h-full`}alt="lines" />))}
