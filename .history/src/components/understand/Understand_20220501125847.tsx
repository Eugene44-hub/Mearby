...
const Understand = () => {
  ...
  return (
    ...
    {  
      darkLine1.map((item:number,index:number)=>(<img key={index} src={lines} className={`absolute ${`md:translate-x-[${item.lg}%]`}   lg:translate-x-[${(item.}%] lg:ml-[10rem]  hidden md:block h-full`}alt="lines" />))}
