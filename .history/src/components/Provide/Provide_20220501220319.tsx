...
const Provide = () => {
  ...
  return (
    <section className='mt-[8.25rem] bg-[#004034]  relative pt-[4.313rem]'>
          {  
      [10,30,50,70,90].map((item:number,index:number)=>(<img key={index} src={lines} className={`hidden lg:block absolute top-0 z-[.1] h-full`} style={{left:`${item}%`}} alt="lines" />))
      }
       {  
      ...
      <div className='container md:pl-[61px] md:pr-[57px] px-[20px]'>
        <div className=''>
          <div className='text-center'>
            2<h2 className='text-white text-[1.5rem] z-[1]'>And if you got service to provide</h2>
          </div>
