...
const Provide = () => {
  ...
  return (
    <section className='mt-[8.25rem] bg-[#004034] relative pt-[4.313rem]'>
          {  
      [20,40,60,80].map((item,index:number)=>(<img key={index} src={lines} className={`absolute top-0 left-[${item}%] hidden md:block h-full`}alt="lines" />))}

{[10,30,50,70,90].map((item,index:number)=>(<img key={index} src={mobilelines} className={`absolute md:hidden block left-[${item}%] h-full`}alt="lines" />))}
