...
const Provide = () => {
  ...
  return (
    <section className='mt-[8.25rem] bg-[#004034] relative pt-[4.313rem]'>
          {  
      [10,30,50,70,90].map((item:number,index:number)=>(<img key={index} src={lines} className={`hidden lg:block absolute top-0  left-[${item}%]  h-full`} style={{left:`${item}%`}} alt="lines" />))
      }
       {  
