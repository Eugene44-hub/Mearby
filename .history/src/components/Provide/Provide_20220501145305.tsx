...
import longLine from "../../assets/images/lines/longestDark.svg"
...
const Provide = () => {
  ...
  return (
    <section className='mt-[8.25rem] bg-[#004034] relative pt-[4.313rem]'>
          {  
      [10,30,50,70,90].map((item:number,index:number)=>(<img key={index} src={lines} className={`hidden lg:block absolute top-0 left-[${item}%]  h-full`}alt="lines" />))
      }
       {  
      [10,30,50,70,90].map((item:number,index:number)=>(<img key={index} src={longLine} className={`lg:hidden absolute top-0 left-[${item}%]  h-full`}alt="lines" />))
      }
      ...
      <div className='container md:pl-[61px] md:pr-[57px] px-[20px]'>
        <div className=' '>
          <div className='text-center '>
            <h2 className='text-white text-[1.5rem] z-50'><span className='And if you got service to provide'>And if you got service to provide</span> </h2>
          </div>
